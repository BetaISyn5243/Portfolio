...
export default function Login(props: LinuxActions) {
  ...
  return (
    <div className={"w-full h-full overflow-hidden bg-center bg-cover"} style={divStyle}>
      <TopBar hide={false} setLogin={()=>props.setLogin}  login={true}/>

      <div
        className="nightwind-prevent w-full h-full  relative top-0 flex flex-col justify-center  items-center text-sm text-white"
        onClick={() => loginHandle()}>
        <div className={"flex-row flex justify-around bg-black bg-opacity-75 rounded shadow-2xl h-32 w-1/5"}>
        ...
        <div className="flex-row flex justify-around bg-black bg-opacity-100 rounded -mt-1 shadow-2xl h-16 w-1/5">
          <div
